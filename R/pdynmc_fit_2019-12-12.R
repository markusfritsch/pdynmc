...
# }
 ...
 class(fit)	<- append(class(fit), "pdynmc.fit")
...
#'    w.mat = "iid.err", std.err = "corrected", estimation = "onestep",
#'    opt.meth = "none")
#' fitted.pdynmc(m1, na.rm = TRUE)
#' }
#'
#'
fitted.pdynmc		<- function(object, step = object$iter, na.rm = FALSE, ...){
  if(na.rm == TRUE){
    fit.pd	<- stats::na.omit(get(paste("step", step, sep = "") , object$fitted))
  } else{
    fit.pd	<- get(paste("step", step, sep = "") , object$fitted)
  }
  return(fit.pd)
}

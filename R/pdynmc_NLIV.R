...
NLIV.T	<- function(
  ...
  y.Tm1[is.na(y.Tm1)]	<- 0
  y.Tm2			          <- data[i = dat[[varname.t]] == sort(unique(dat[[varname.t]]))[length(unique(as.numeric(dat[[varname.t]]))) - 2], j = y]
  y.Tm2[is.na(y.Tm2)]	<- 0
  y.2				          <- dat[i = dat[[varname.t]] == sort(unique(dat[[varname.t]]))[2], j = y]
  y.2[is.na(y.2)]		  <- 0
  y.1				          <- data[i = dat[[varname.t]] == sort(unique(dat[[varname.t]]))[1], j = y]
  y.1[is.na(y.1)]		  <- 0
...
NLIV.t	<- function(
  ...
  dat			  <- merge(x = dat_b, y = dat, by = c(varname.i, varname.t), all.x = TRUE)
  dat			  <- data[order(dat[[varname.i]], dat[[varname.t]], decreasing = FALSE), ]
...
AH81	<- function (
  ...
  dat			    <- merge(x = dat_b, y = data, by = c(varname.i, varname.t), all.x = TRUE)
  dat			    <- dat[order(dat[[varname.i]], dat[[varname.t]], decreasing = FALSE), ]

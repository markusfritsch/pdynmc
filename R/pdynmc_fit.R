...
##  stat		<- as.numeric(tcrossprod(t(tzu), crossprod(tzu, as.vector(Matrix::crossprod(do.call(Szero.j, what = "c"), do.call(Szero.j, what = "c"), na.rm = TRUE)))))
...
##				((as.vector(Matrix::crossprod(do.call(Szero.j, what = "c"), do.call(Szero.j, what = "c"), na.rm = TRUE)) /(sum(!is.na(diff(dat.na[, varname.y],2))) - sum(n.inst))))^(-1)		#[M:] adjusted dof correction (unbalancedness of the data is accounted for)
